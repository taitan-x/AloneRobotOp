...
from AloneRobot import DRAGONS
from AloneRobot.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
...
async def Alone_is_admin(chat_id: int):
    status = False
    Alone = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id, filter=ChannelParticipantsAdmins
    ):
        if Alone.id == user.id:
